...
int load_(struct treenode_st **root, FILE *fp)
{
	char letter;
	struct treenode_st *node = NULL;

	letter = fgetc(fp);
	//printf("letter=%c\n", letter);
	if(letter != '(')
	{
		fprintf(stderr, "error format of the file. it should start with '('\n");
		return -1;
	}
	...
	*root = node;
	...
	load_(&node->leftChild, fp);
	load_(&node->rightChild, fp);

	
	letter = fgetc(fp);
	if(letter != ')')
	{
		fprintf(stderr, "error format of the file. it should end with ')'\n");
		return -2;
	}
...
void load(struct treenode_st **root)
{
	FILE *fp = NULL;
	fp = fopen(FILENAME, "r");
	load_(root, fp);
